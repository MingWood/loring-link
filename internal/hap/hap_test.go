...
// TestBuildDropButtonFrameMatchesCapturedExamples validates against the
// captured drop-button press/release pair
// (dropbuttonlightPressfirsttwomsgsAndThenLongPresstoDrop.txt, frames 234 and
// 252): press checksum 0x898f with value=1, release checksum 0xb8bc with
// value=0, both at the shared button address 87 01 33.
func TestBuildDropButtonFrameMatchesCapturedExamples(t *testing.T) {
	press := BuildDropButtonFrame(true, []byte{0x8f, 0x23})
	wantPress := []byte{
		'H', 'A', 'P', 0x8f, 0x23, 0x89, 0x8f,
		0x0a, 0x00, 0x19, 0x00, 0x01, 0x20, 0x02,
		0x87, 0x01, 0x33,
		0x01, 0x00,
	}
	...
	release := BuildDropButtonFrame(false, []byte{0x95, 0x23})
	wantRelease := []byte{
		'H', 'A', 'P', 0x95, 0x23, 0xb8, 0xbc,
		0x0a, 0x00, 0x19, 0x00, 0x01, 0x20, 0x02,
		0x87, 0x01, 0x33,
		0x00, 0x00,
	}
